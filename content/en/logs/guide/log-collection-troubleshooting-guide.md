...
5. The name of the directory in `the conf.d/` folder must match the service value specified in the `conf.yaml` file within that directory. For example, for `conf.d/<APP_NAME>/conf.yaml` the `conf.yaml` file has `service` set to `APP_NAME`:
